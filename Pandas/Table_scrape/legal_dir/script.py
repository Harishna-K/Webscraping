...
# for x in range(0, 1766): 
#     url = f'https://www.mywsba.org/PersonifyEbusiness/LegalDirectory.aspx?ShowSearchResults=TRUE&Status=Active&Page={x}'
#     try:
...
#             print(f"Page {x} scraped successfully.")
#         else:
#             print(f"No table found on page {x}")
#     except Exception as e:
#         print(f"Failed to scrape page {x}: {e}")

# final_df = pd.concat(all_tables, index=False)
...
# Function to scrape a single page with retries for failed pages
