...
url = 'https://www.dealsonwheelsnz.co.nz/vehicles?Page=1'
base_url = 'https://www.dealsonwheelsnz.co.nz'
...
#                 if len(each_page) < 20:
#                     print(f"Less than 20 cars in page: {page}")
#                     Less_pages.append(page)
                
#                 if not each_page:
#                     print(f"No more cars found in page:{page}")
#                     continue
...
#                 print(f"Giving upon page: {page}, attempts {attempt + 1}/{Retries}")
...
for each_file in glob.glob("Storage/*.html"):
    with open(each_file, "rb") as fp:
        ...
        price = tree.xpath('//span[@class="price"]//span/span[@class="amount"]/text()')[0].strip()
